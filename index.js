...
function filterByCondition(arr, condition) {
  ...
  const filterArr = arr.filter(condition);
...
function findElementIndexes(arr, element) {
  ...
  const arr1 = arr.indexOf(element);
  const arr2 = arr.lastIndexOf(element);
...
function customShift(arr) {
  ...
  return `{shiftedElement: ${arr.shift()}, arr: [${arr}]}`;
}
...
function customUnshift(arr, ...elements) {
  ...
  for (let i = 0; i < arr.length; i++) {
    return `{initialLength: ${initialLength}, newLength: ${arr.unshift(
      ...elements
    )}, arr: [${arr}]}`;
  }
}
...
function customAt(arr, index) {
  ...
  if (typeof arr === "") {
    return `Елемент є рядком ${arr.at(index)}`;
  } else if (typeof arr) {
    return `Елемент є числом ${arr.at(index)}`;
  } else {
...
function customIncludes(arr, element) {
  ...
  const filteredArr = arr.filter((item) => item === element);
